...
function transformer(file, api) {
  ...
  const root = j(file.source); //  Create a dictionary whose keys are all the named imports in the file.
  //  For instance:
  //
  //  import foo from 'foo';
  ...
  const importMap = {};
  root.find(j.ImportDeclaration).forEach(imp => imp.node.specifiers.forEach(spec => {
  ...
  const storiesOfCalls = root.find(j.CallExpression).filter(call => call.node.callee.name === 'storiesOf').filter(call => call.node.arguments.length > 0 && call.node.arguments[0].type === 'Literal').filter(call => {
