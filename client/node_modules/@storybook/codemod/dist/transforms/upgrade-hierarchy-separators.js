...
function transformer(file, api, options) {
  ...
  const root = j(file.source); // storiesOf(...)

  root.find(j.CallExpression).filter(call => call.node.callee.name === 'storiesOf').filter(call => call.node.arguments.length > 0 && ['Literal', 'StringLiteral'].includes(call.node.arguments[0].type)).forEach(call => {
